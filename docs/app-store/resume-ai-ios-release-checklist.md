...
- [x] Fresh release-sweep evidence on 2026-08-11: 754 backend/configuration tests passed with 6 real-Redis tests explicitly skipped locally (the earlier clean-checkout run passed those with temporary Redis 8.10); mobile 30 suites / 668 tests passed after removing the dormant Apple-authentication client and its three tests; TypeScript, lint, Expo Doctor 18/18, the release-asset gate, and a 5.0 MB Hermes iOS export passed. A fresh production prebuild generated empty entitlements with no Sign in with Apple capability.
...
- [ ] `BLOCKED` — earlier anonymous probes on 2026-08-11 returned HTTP 200 from the legacy analyzer root and HTTP 404 at `/healthz`; later 25-second and 90-second probes to both endpoints timed out with zero response bytes. Deploy the current service, then pass health and a synthetic complete analysis flow anonymously.
