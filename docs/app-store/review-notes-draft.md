...
All three URLs returned HTTPS 200 anonymously on 2026-08-14 and matched their tracked release files byte for byte. Recheck them immediately before submission. On the same date, the live Render root returned HTTP 200 but served a March 29, 2026 legacy artifact with wildcard CORS and no HSTS; `/healthz` returned HTTP 404. Deploy and verify the current service before review. Signed PDFKit/Vision device evidence, Apple Sandbox, RevenueCat webhooks, production-like load evidence, TestFlight, and App Review remain external gates. These notes have not been submitted and no Apple approval is claimed.
