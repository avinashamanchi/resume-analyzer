...
All three URLs returned HTTPS 200 anonymously on 2026-08-11 and matched their tracked release files byte for byte. Recheck them immediately before submission. Earlier anonymous Render probes that day served the legacy analyzer root and returned HTTP 404 at `/healthz`; later 25-second and 90-second probes to both endpoints timed out with zero response bytes. Deploy and verify the current service before review. Signed PDFKit/Vision device evidence, Apple Sandbox, RevenueCat webhooks, production-like load evidence, TestFlight, and App Review remain external gates. These notes have not been submitted and no Apple approval is claimed.
