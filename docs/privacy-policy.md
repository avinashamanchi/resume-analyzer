...
Resume.AI uses an installation security identifier and a coarse pseudonymous rate-limit key to protect the service. Those identifiers are not used for advertising, analytics, cross-app tracking, or user profiling. There are no ads, cross-app tracking, or behavioral/product-interaction analytics. RevenueCat processes purchase history for subscription analytics and app functionality using a RevenueCat-generated anonymous app-user identifier; Resume.AI supplies no custom account identifier, does not link that purchase history to the user, and does not use it for tracking.
